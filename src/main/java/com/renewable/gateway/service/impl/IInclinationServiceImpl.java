...
//        iInclinationDeal.sendData("COM1",9600,01, InclinationConst.InclinationSensor1Enum.READALL);
	...
	public ServerResponse receiveData(String port, int baudrate, byte[] originBuffer) {
		...
		// 判断初始配置表的状态，决定是否进行计算
		int statusCode = initializationInclination.getStatus();
		if (statusCode != InitializationConstant.InitializationStatusEnum.USE.getCode()){
			return ServerResponse.createByErrorMessage(InitializationConstant.InitializationStatusEnum.codeOf(statusCode).getValue());
